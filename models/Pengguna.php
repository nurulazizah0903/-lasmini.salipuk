 ...
 */
class Pengguna extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'pengguna';
        $this->TableName = 'pengguna';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`pengguna`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->upt_id->InputTextType = "text";
        $this->upt_id->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->upt_id->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->upt_id->Lookup = new Lookup('upt_id', 'upt', false, 'id', ["nama","","",""], [], [], [], [], [], [], '', '', "`nama`");
        $this->upt_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['upt_id'] = &$this->upt_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`pengguna`";
    }
    ...
    public function getSqlSelectList() // Select for List page
    {
        ...
        $from = "(SELECT *, (SELECT `nama` FROM `upt` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id` = `pengguna`.`upt_id` LIMIT 1) AS `EV__upt_id` FROM `pengguna`)";
    ...
    protected function useVirtualFields()
    {
        ...
        if (ContainsString($orderBy, " " . $this->upt_id->VirtualExpression . " ")) {
            return true;
        }
        return false;
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->upt_id->DbValue = $row['upt_id'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("PenggunaList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "PenggunaEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "PenggunaAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("PenggunaView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("PenggunaView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "PenggunaAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "PenggunaAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("PenggunaEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PenggunaAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("PenggunaDelete", $this->getUrlParm());
    }
    ...
    public function recordsetSelecting(&$filter)
    {
        ...
        $upt_id = CurrentUserInfo('id');
        $level_id = CurrentUserLevel();
        if($level_id == 2){
            AddFilter($filter, "id = $upt_id");
        }
    }
