...
class BroadcastList extends Broadcast
{
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["broadcast"]) || get_class($GLOBALS["broadcast"]) == PROJECT_NAMESPACE . "broadcast") {
        ...
        // Initialize URLs
        $this->AddUrl = "BroadcastAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
    ...
    protected function getRecordsFromRecordset($rs, $current = false)
    {
        ...
        if (is_object($rs)) { // Recordset
            while ($rs && !$rs->EOF) {
		        ...
		        $this->file->OldUploadPath = "broadcast";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fbroadcastsrch", $filters);
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_broadcast_tujuan"
        $item = &$this->ListOptions->add("detail_broadcast_tujuan");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'broadcast_tujuan');
        $item->OnLeft = false;
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbroadcastlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fbroadcastlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_broadcast_tujuan"
        $opt = $this->ListOptions["detail_broadcast_tujuan"];
        if ($Security->allowList(CurrentProjectID() . 'broadcast_tujuan')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("broadcast_tujuan", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("BroadcastTujuanList?" . Config("TABLE_SHOW_MASTER") . "=broadcast&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("BroadcastTujuanGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'broadcast')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=broadcast_tujuan");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "broadcast_tujuan";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'broadcast')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=broadcast_tujuan");
                ...
                $detailEditTblVar .= "broadcast_tujuan";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'broadcast')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=broadcast_tujuan");
                ...
                $detailCopyTblVar .= "broadcast_tujuan";
            }
            if ($links != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_broadcast_tujuan");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=broadcast_tujuan");
                $detailPage = Container("BroadcastTujuanGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'broadcast') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "broadcast_tujuan";
                }
    ...
    protected function newRow()
    {
        ...
        $row['text'] = $this->text->DefaultValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fbroadcastsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
