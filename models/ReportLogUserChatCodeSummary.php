 ...
 * Page class
 */
class ReportLogUserChatCodeSummary extends ReportLogUserChatCode
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'Report Log User Chat Code';

    // Page object name
    public $PageObjName = "ReportLogUserChatCodeSummary";
    ...
    public function __construct()
    {
        ...
        // Table object (ReportLogUserChatCode)
        if (!isset($GLOBALS["ReportLogUserChatCode"]) || get_class($GLOBALS["ReportLogUserChatCode"]) == PROJECT_NAMESPACE . "ReportLogUserChatCode") {
            $GLOBALS["ReportLogUserChatCode"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Report Log User Chat Code');
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_SEARCH) {
            // pelapor_id
            if ($this->pelapor_id->UseFilter && !EmptyValue($this->pelapor_id->AdvancedSearch->SearchValue)) {
                if (is_array($this->pelapor_id->AdvancedSearch->SearchValue)) {
                    $this->pelapor_id->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->pelapor_id->AdvancedSearch->SearchValue);
                }
                $this->pelapor_id->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->pelapor_id->AdvancedSearch->SearchValue);
            }

            // pesan
            if ($this->pesan->UseFilter && !EmptyValue($this->pesan->AdvancedSearch->SearchValue)) {
                if (is_array($this->pesan->AdvancedSearch->SearchValue)) {
                    $this->pesan->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->pesan->AdvancedSearch->SearchValue);
                }
                $this->pesan->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->pesan->AdvancedSearch->SearchValue);
            }
            ...
            $this->pesan->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
        ...
        if ($this->RowType == ROWTYPE_TOTAL) { // Summary row
            ...
            // pesan
            $currentValue = $this->pesan->CurrentValue;
            $viewValue = &$this->pesan->ViewValue;
            $viewAttrs = &$this->pesan->ViewAttrs;
            $cellAttrs = &$this->pesan->CellAttrs;
            $hrefValue = &$this->pesan->HrefValue;
            $linkAttrs = &$this->pesan->LinkAttrs;
            $this->cellRendered($this->pesan, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fReportLogUserChatCodesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->id->Visible || $this->pelapor_id->Visible || $this->pesan->Visible || $this->tanggal->Visible || $this->code->Visible;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fReportLogUserChatCodesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fReportLogUserChatCodesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    protected function getExtendedFilter()
    {
        ...
        if (Get("cmd", "") == "reset") {
            // Set default values
            $this->pelapor_id->AdvancedSearch->unsetSession();
            ...
            // Field pelapor_id
            $this->getDropDownValue($this->pelapor_id);

            // Field pesan
            $this->getDropDownValue($this->pesan);
        ...
        if ($restoreSession) {
            $restoreDefault = true;
            if ($this->pelapor_id->AdvancedSearch->issetSession()) { // Field pelapor_id
                $this->pelapor_id->AdvancedSearch->load();
                $restoreDefault = false;
            }
            if ($this->pesan->AdvancedSearch->issetSession()) { // Field pesan
        ...
        $this->buildDropDownFilter($this->pelapor_id, $filter, false, true); // Field pelapor_id
        $this->pelapor_id->AdvancedSearch->save();
        $this->buildDropDownFilter($this->pesan, $filter, false, true); // Field pesan
