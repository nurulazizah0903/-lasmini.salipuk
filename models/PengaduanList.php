 ...
 * Page class
 */
class PengaduanList extends Pengaduan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PengaduanList";
    ...
    public $FormName = "fpengaduanlist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "PengaduanAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "PengaduanDelete";
        $this->MultiUpdateUrl = "PengaduanUpdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->pelapor_id->AdvancedSearch->toJson(), ","); // Field pelapor_id
        $filterList = Concat($filterList, $this->jalan_id->AdvancedSearch->toJson(), ","); // Field jalan_id
        $filterList = Concat($filterList, $this->foto->AdvancedSearch->toJson(), ","); // Field foto
        $filterList = Concat($filterList, $this->kordinat->AdvancedSearch->toJson(), ","); // Field kordinat
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fpengaduansrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field pelapor_id
        $this->pelapor_id->AdvancedSearch->SearchValue = @$filter["x_pelapor_id"];
        $this->pelapor_id->AdvancedSearch->SearchOperator = @$filter["z_pelapor_id"];
        $this->pelapor_id->AdvancedSearch->SearchCondition = @$filter["v_pelapor_id"];
        $this->pelapor_id->AdvancedSearch->SearchValue2 = @$filter["y_pelapor_id"];
        $this->pelapor_id->AdvancedSearch->SearchOperator2 = @$filter["w_pelapor_id"];
        $this->pelapor_id->AdvancedSearch->save();

        // Field jalan_id
        $this->jalan_id->AdvancedSearch->SearchValue = @$filter["x_jalan_id"];
        $this->jalan_id->AdvancedSearch->SearchOperator = @$filter["z_jalan_id"];
        $this->jalan_id->AdvancedSearch->SearchCondition = @$filter["v_jalan_id"];
        $this->jalan_id->AdvancedSearch->SearchValue2 = @$filter["y_jalan_id"];
        $this->jalan_id->AdvancedSearch->SearchOperator2 = @$filter["w_jalan_id"];
        ...
        // Field kordinat
        $this->kordinat->AdvancedSearch->SearchValue = @$filter["x_kordinat"];
        $this->kordinat->AdvancedSearch->SearchOperator = @$filter["z_kordinat"];
        $this->kordinat->AdvancedSearch->SearchCondition = @$filter["v_kordinat"];
        $this->kordinat->AdvancedSearch->SearchValue2 = @$filter["y_kordinat"];
        $this->kordinat->AdvancedSearch->SearchOperator2 = @$filter["w_kordinat"];
        $this->kordinat->AdvancedSearch->save();
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fpengaduanlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fpengaduanlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("pelapor_id", $this->createColumnOption("pelapor_id"));
            $option->add("jalan_id", $this->createColumnOption("jalan_id"));
            $option->add("foto", $this->createColumnOption("foto"));
            $option->add("keterangan", $this->createColumnOption("keterangan"));
            $option->add("token", $this->createColumnOption("token"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpengaduansrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpengaduansrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpengaduanlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpengaduansrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
