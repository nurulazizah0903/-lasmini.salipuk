...
/**
 * Table class for pengaduan
 */
class Pengaduan extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->jalan_id->Lookup = new Lookup('jalan_id', 'jalan', false, 'id', ["nama","","",""], [], [], [], [], [], [], '', '', "`nama`");
        ...
        $this->status->Lookup = new Lookup('status', 'pengaduan', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        ...
        $this->waktu = new DbField(
            ...
            'waktu',
            '`waktu`',
            CastDateFieldForLike("`waktu`", 1, "DB"),
            135,
            19,
            1,
            false,
        ...
        $this->waktu->DefaultErrorMessage = str_replace("%s", DateFormat(1), $Language->phrase("IncorrectDate"));
    ...
    public function getSqlSelectList() // Select for List page
    {
        ...
        $from = "(SELECT *, (SELECT `nama` FROM `pelapor` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`id` = `pengaduan`.`pelapor_id` LIMIT 1) AS `EV__pelapor_id` FROM `pengaduan`)";
    ...
    protected function useVirtualFields()
    {
        ...
        if (
            $this->pelapor_id->AdvancedSearch->SearchValue != "" ||
            $this->pelapor_id->AdvancedSearch->SearchValue2 != "" ||
            ContainsString($where, " " . $this->pelapor_id->VirtualExpression . " ")
        ) {
            return true;
        }
        if (ContainsString($orderBy, " " . $this->pelapor_id->VirtualExpression . " ")) {
            return true;
        }
        return false;
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->jalan_id->DbValue = $row['jalan_id'];
        $this->foto->DbValue = $row['foto'];
        $this->kordinat->DbValue = $row['kordinat'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("PengaduanList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "PengaduanEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "PengaduanAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_EDIT_ACTION"):
                return "PengaduanEdit";
            case Config("API_DELETE_ACTION"):
                return "PengaduanDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("PengaduanView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("PengaduanView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "PengaduanAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "PengaduanAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("PengaduanEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PengaduanAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("PengaduanDelete", $this->getUrlParm());
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->kordinat->EditValue = $this->kordinat->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->jalan_id);
                    $doc->exportCaption($this->foto);
                    $doc->exportCaption($this->kordinat);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->jalan_id);
                        $doc->exportField($this->foto);
                        $doc->exportField($this->kordinat);
    ...
    public function recordsetSelecting(&$filter)
    {
        ...
        $upt_id = CurrentUserInfo('upt_id');
        if($upt_id){
            AddFilter($filter, "jalan_id IN (SELECT id FROM jalan where upt_id = $upt_id)");
        }    
    }
