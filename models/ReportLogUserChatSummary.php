 ...
 * Page class
 */
class ReportLogUserChatSummary extends ReportLogUserChat
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'Report Log User Chat';

    // Page object name
    public $PageObjName = "ReportLogUserChatSummary";
    ...
    public function __construct()
    {
        ...
        // Table object (ReportLogUserChat)
        if (!isset($GLOBALS["ReportLogUserChat"]) || get_class($GLOBALS["ReportLogUserChat"]) == PROJECT_NAMESPACE . "ReportLogUserChat") {
            $GLOBALS["ReportLogUserChat"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Report Log User Chat');
        }
        ...
        if ($this->RowType == ROWTYPE_SEARCH) {
            ...
            if ($this->tanggal->UseFilter && !EmptyValue($this->tanggal->AdvancedSearch->SearchValue)) {
                if (is_array($this->tanggal->AdvancedSearch->SearchValue)) {
                    $this->tanggal->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->tanggal->AdvancedSearch->SearchValue);
                }
                $this->tanggal->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->tanggal->AdvancedSearch->SearchValue);
            }
            ...
            $this->tanggal->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fReportLogUserChatsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fReportLogUserChatsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fReportLogUserChatsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    protected function getExtendedFilter()
    {
        ...
        if (Get("cmd", "") == "reset") {
            ...
            // Field tanggal
            $this->getDropDownValue($this->tanggal);
        ...
        if ($restoreSession) {
            ...
            if ($this->tanggal->AdvancedSearch->issetSession()) { // Field tanggal
        ...
        $this->buildDropDownFilter($this->tanggal, $filter, false, true); // Field tanggal
