 ...
 * Page class
 */
class LogUserChatAdd extends LogUserChat
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "LogUserChatAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("LogUserChatList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "LogUserChatList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "LogUserChatView") {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // pesan
            $this->pesan->setupEditAttributes();
            $this->pesan->EditCustomAttributes = "";
            if (!$this->pesan->Raw) {
                $this->pesan->CurrentValue = HtmlDecode($this->pesan->CurrentValue);
            }
            $this->pesan->EditValue = HtmlEncode($this->pesan->CurrentValue);
            $curVal = strval($this->pesan->CurrentValue);
            if ($curVal != "") {
                $this->pesan->EditValue = $this->pesan->lookupCacheOption($curVal);
                if ($this->pesan->EditValue === null) { // Lookup from database
                    $filterWrk = "`code`" . SearchString("=", $curVal, DATATYPE_STRING, "");
                    $sqlWrk = $this->pesan->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->pesan->Lookup->renderViewRow($rswrk[0]);
                        $this->pesan->EditValue = $this->pesan->displayValue($arwrk);
                    } else {
                        $this->pesan->EditValue = HtmlEncode($this->pesan->CurrentValue);
                    }
                }
            } else {
                $this->pesan->EditValue = null;
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->pesan->Required) {
            if (!$this->pesan->IsDetailKey && EmptyValue($this->pesan->FormValue)) {
                $this->pesan->addErrorMessage(str_replace("%s", $this->pesan->caption(), $this->pesan->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // tanggal
        $this->tanggal->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal->CurrentValue, $this->tanggal->formatPattern()), null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("LogUserChatList"), "", $this->TableVar, true);
