...
loadjs.ready("head", function () {
    ...
    $(".login-box-msg").css(`font-size`, '21px');
    $(".login-box-msg").css(`font-weight`, 'bolder');
    $(".login-box-msg").css(`color`, 'white');
    $(".login-box-msg").html(`Disdukcapil Chatbot`);
});
</script>
<style>
    body{
        background: url('./LASMINII-web (1).jpg');
...
<div class="ew-login-box">
    <div class="login-logo">
        <img src="./LASMINII-letter.png" style="width: 100%; margin-top: 28px;" alt="">
    </div>
    <div class="card ew-login-card" style="background: transparent; border: none;">
        <div class="card-body">
<script>
...
<form name="flogin" id="flogin" class="ew-form ew-login-form" action="<?= CurrentPageUrl(false) ?>" method="post">
    ...
    <div class="d-grid">
        <button style="background-color: #c19139;border-color: #c19139;" class="btn btn-primary ew-btn" name="btn-submit" id="btn-submit" type="submit" formaction="<?= CurrentPageUrl(false) ?>"><?= $Language->phrase("Login", true) ?></button>
    </div>
<?php
// OAuth login
