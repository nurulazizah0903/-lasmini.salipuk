...
// Page object
$KordinatJalanEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { kordinat_jalan: currentTable } });
var currentForm, currentPageID;
var fkordinat_jalanedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fkordinat_jalanedit = new ew.Form("fkordinat_jalanedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fkordinat_jalanedit;

    // Add fields
    var fields = currentTable.fields;
    fkordinat_jalanedit.addFields([
    ...
    // Form_CustomValidate
    fkordinat_jalanedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fkordinat_jalanedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fkordinat_jalanedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fkordinat_jalanedit" id="fkordinat_jalanedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php } ?>
    ...
    <input type="hidden" data-table="kordinat_jalan" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
