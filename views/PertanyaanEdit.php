...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpertanyaanedit = new ew.Form("fpertanyaanedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fpertanyaanedit;

    // Add fields
    var fields = currentTable.fields;
    fpertanyaanedit.addFields([
    ...
    // Form_CustomValidate
    fpertanyaanedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpertanyaanedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpertanyaanedit.lists.pid = <?= $Page->pid->toClientList($Page) ?>;
    loadjs.done("fpertanyaanedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fpertanyaanedit" id="fpertanyaanedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_pertanyaan_pid">
    <select
        ...
        data-select2-id="fpertanyaanedit_x_pid"
...
<script>
loadjs.ready("fpertanyaanedit", function() {
    var options = { name: "x_pid", selectId: "fpertanyaanedit_x_pid" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpertanyaanedit.lists.pid.lookupOptions.length) {
        options.data = { id: "x_pid", form: "fpertanyaanedit" };
    } else {
        options.ajax = { id: "x_pid", form: "fpertanyaanedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<input type="<?= $Page->nama->getInputTextType() ?>" name="x_nama" id="x_nama" data-table="pertanyaan" data-field="x_nama" value="<?= $Page->nama->EditValue ?>" size="30" maxlength="50" placeholder="<?= HtmlEncode($Page->nama->getPlaceHolder()) ?>"<?= $Page->nama->editAttributes() ?> aria-describedby="x_nama_help">
...
<?php } ?>
    ...
    <input type="hidden" data-table="pertanyaan" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
