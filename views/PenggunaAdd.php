...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpenggunaadd = new ew.Form("fpenggunaadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fpenggunaadd;

    // Add fields
    var fields = currentTable.fields;
    fpenggunaadd.addFields([
    ...
    // Form_CustomValidate
    fpenggunaadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpenggunaadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpenggunaadd.lists.upt_id = <?= $Page->upt_id->toClientList($Page) ?>;
    fpenggunaadd.lists.level = <?= $Page->level->toClientList($Page) ?>;
    loadjs.done("fpenggunaadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fpenggunaadd" id="fpenggunaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_pengguna_upt_id">
    <select
        ...
        data-select2-id="fpenggunaadd_x_upt_id"
        data-table="pengguna"
        data-field="x_upt_id"
...
<script>
loadjs.ready("fpenggunaadd", function() {
    var options = { name: "x_upt_id", selectId: "fpenggunaadd_x_upt_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpenggunaadd.lists.upt_id.lookupOptions.length) {
        options.data = { id: "x_upt_id", form: "fpenggunaadd" };
    } else {
        options.ajax = { id: "x_upt_id", form: "fpenggunaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_pengguna_level">
    <select
        ...
        data-select2-id="fpenggunaadd_x_level"
        data-table="pengguna"
        data-field="x_level"
...
<script>
loadjs.ready("fpenggunaadd", function() {
    var options = { name: "x_level", selectId: "fpenggunaadd_x_level" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpenggunaadd.lists.level.lookupOptions.length) {
        options.data = { id: "x_level", form: "fpenggunaadd" };
    } else {
        options.ajax = { id: "x_level", form: "fpenggunaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
