...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fpengaduanedit = new ew.Form("fpengaduanedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fpengaduanedit;

    // Add fields
    var fields = currentTable.fields;
    fpengaduanedit.addFields([
    ...
    // Form_CustomValidate
    fpengaduanedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpengaduanedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fpengaduanedit.lists.pelapor_id = <?= $Page->pelapor_id->toClientList($Page) ?>;
    fpengaduanedit.lists.jalan_id = <?= $Page->jalan_id->toClientList($Page) ?>;
    fpengaduanedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    loadjs.done("fpengaduanedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fpengaduanedit" id="fpengaduanedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_pengaduan_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_pengaduan_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="pengaduan" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_pengaduan_pelapor_id">
    <select
        ...
        data-select2-id="fpengaduanedit_x_pelapor_id"
...
<script>
loadjs.ready("fpengaduanedit", function() {
    var options = { name: "x_pelapor_id", selectId: "fpengaduanedit_x_pelapor_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpengaduanedit.lists.pelapor_id.lookupOptions.length) {
        options.data = { id: "x_pelapor_id", form: "fpengaduanedit" };
    } else {
        options.ajax = { id: "x_pelapor_id", form: "fpengaduanedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_pengaduan_jalan_id">
    <select
        ...
        data-select2-id="fpengaduanedit_x_jalan_id"
        data-table="pengaduan"
...
<script>
loadjs.ready("fpengaduanedit", function() {
    var options = { name: "x_jalan_id", selectId: "fpengaduanedit_x_jalan_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpengaduanedit.lists.jalan_id.lookupOptions.length) {
        options.data = { id: "x_jalan_id", form: "fpengaduanedit" };
    } else {
        options.ajax = { id: "x_jalan_id", form: "fpengaduanedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_pengaduan_status">
    <select
        ...
        data-select2-id="fpengaduanedit_x_status"
        data-table="pengaduan"
        data-field="x_status"
...
<script>
loadjs.ready("fpengaduanedit", function() {
    var options = { name: "x_status", selectId: "fpengaduanedit_x_status" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpengaduanedit.lists.status.lookupOptions.length) {
        options.data = { id: "x_status", form: "fpengaduanedit" };
    } else {
        options.ajax = { id: "x_status", form: "fpengaduanedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fpengaduanedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpengaduanedit", "x_waktu", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
