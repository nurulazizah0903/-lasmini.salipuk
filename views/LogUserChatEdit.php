...
// Page object
$LogUserChatEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { log_user_chat: currentTable } });
var currentForm, currentPageID;
var flog_user_chatedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    flog_user_chatedit = new ew.Form("flog_user_chatedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = flog_user_chatedit;

    // Add fields
    var fields = currentTable.fields;
    flog_user_chatedit.addFields([
    ...
    // Form_CustomValidate
    flog_user_chatedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    flog_user_chatedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    flog_user_chatedit.lists.pelapor_id = <?= $Page->pelapor_id->toClientList($Page) ?>;
    flog_user_chatedit.lists.pesan = <?= $Page->pesan->toClientList($Page) ?>;
    loadjs.done("flog_user_chatedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="flog_user_chatedit" id="flog_user_chatedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_log_user_chat_pelapor_id">
    <select
        ...
        data-select2-id="flog_user_chatedit_x_pelapor_id"
...
<script>
loadjs.ready("flog_user_chatedit", function() {
    var options = { name: "x_pelapor_id", selectId: "flog_user_chatedit_x_pelapor_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (flog_user_chatedit.lists.pelapor_id.lookupOptions.length) {
        options.data = { id: "x_pelapor_id", form: "flog_user_chatedit" };
    } else {
        options.ajax = { id: "x_pelapor_id", form: "flog_user_chatedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready("flog_user_chatedit", function() {
    flog_user_chatedit.createAutoSuggest(Object.assign({"id":"x_pesan","forceSelect":false}, ew.vars.tables.log_user_chat.fields.pesan.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["flog_user_chatedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("flog_user_chatedit", "x_tanggal", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="log_user_chat" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
