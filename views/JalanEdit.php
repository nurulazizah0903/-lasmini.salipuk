...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fjalanedit = new ew.Form("fjalanedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fjalanedit;

    // Add fields
    var fields = currentTable.fields;
    fjalanedit.addFields([
    ...
    // Form_CustomValidate
    fjalanedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fjalanedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fjalanedit.lists.upt_id = <?= $Page->upt_id->toClientList($Page) ?>;
    loadjs.done("fjalanedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fjalanedit" id="fjalanedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_jalan_upt_id">
    <select
        ...
        data-select2-id="fjalanedit_x_upt_id"
        data-table="jalan"
        data-field="x_upt_id"
...
<script>
loadjs.ready("fjalanedit", function() {
    var options = { name: "x_upt_id", selectId: "fjalanedit_x_upt_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjalanedit.lists.upt_id.lookupOptions.length) {
        options.data = { id: "x_upt_id", form: "fjalanedit" };
    } else {
        options.ajax = { id: "x_upt_id", form: "fjalanedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="jalan" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
